...
def encoder_forward(lstm1, lstm2, samples, vocab, tag_to_ind_map, bs, dropout_prob, train=True, \
	...
	for x_vecs, batch_to_sample, batch_to_span_ind in prepare_edus_seq_as_inp_vecs(\
		...
		set_encoded_edus(encoded_edus, batch_to_sample, batch_to_span_ind)
...
def set_encoded_edus(encoded_edus, batch_to_sample, batch_to_span_ind):
	for i in range(len(encoded_edus)):
		sample = batch_to_sample[i]
		span_ind = batch_to_span_ind[i]
		s, t = sample._spans[span_ind]
		if sample._encoded_spans == []:
			sample._encoded_spans = [0] * (len(sample._spans))
		encoded_span = 0
		for k in range(s, t + 1):
			encoded_span += encoded_edus[i, k - s, :]
		sample._encoded_spans[span_ind] = encoded_span / (t - s + 1)
